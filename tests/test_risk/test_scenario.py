...
def test_non_parallel_steepening():
    ...
    shocked_curve = generator.apply_non_parallel_shift(base_curve, shift_type='steepening', alpha=0.01)
...
def test_non_parallel_flattening():
    ...
    shocked_curve = generator.apply_non_parallel_shift(base_curve, shift_type='flattening', alpha=0.01)
...
def test_non_parallel_twist():
    ...
    shocked_curve = generator.apply_non_parallel_shift(base_curve, shift_type='twist', alpha=0.01, pivot=5)
    
    # Yield at pivot should remain roughly unchanged
    np.testing.assert_almost_equal(shocked_curve.yields[2], base_curve.yields[2], decimal=4)
    # Short term should change in opposite direction to long term
    assert np.sign(shocked_curve.yields[0] - base_curve.yields[0]) != np.sign(shocked_curve.yields[-1] - base_curve.yields[-1])
