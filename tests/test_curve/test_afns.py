...
def test_afns_initialization():
    afns = ArbitrageFreeNelsonSiegel(beta0=0.05, beta1=-0.02, beta2=0.01, tau=1.5, sigma=0.01)
    assert afns.beta0 == 0.05
    assert afns.sigma == 0.01
    
    with pytest.raises(ValueError):
        ArbitrageFreeNelsonSiegel(beta0=0.05, beta1=-0.02, beta2=0.01, tau=1.5, sigma=-0.01)


def test_afns_convexity_adjustment():
    afns = ArbitrageFreeNelsonSiegel(beta0=0.05, beta1=-0.02, beta2=0.01, tau=1.5, sigma=0.05)
...
def test_afns_yield_rate():
    afns = ArbitrageFreeNelsonSiegel(beta0=0.05, beta1=0.0, beta2=0.0, tau=1.5, sigma=0.01)
