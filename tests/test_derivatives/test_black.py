...
def test_black_formula():
    ...
    call_px = black_formula(fwd, strike, t_exp, sigma, df, is_call=True)
    put_px = black_formula(fwd, strike, t_exp, sigma, df, is_call=False)

    # Put-Call Parity: Call - Put = df * (F - K)
    assert (call_px - put_px) == pytest.approx(df * (fwd - strike), abs=1e-6)
    assert call_px > 0
    assert put_px > 0
...
def test_black_cap_floor_parity():
    ...
    # Cap - Floor = Payer Swap (standard cap-floor parity)
...
def test_black_swaption_parity():
    ...
    assert (pv_payer - pv_receiver) == pytest.approx(
