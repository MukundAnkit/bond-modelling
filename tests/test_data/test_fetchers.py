...
from src.data.exceptions import DataFetchError, DataNormalizationError, MarketHolidayError
...
def test_fred_fetcher_api_error():
    fetcher = FredFetcher()
    with patch("src.data.fred.web.DataReader", side_effect=RequestException("API down")):
...
def test_yfinance_fetcher_success():
    ...
    # yfinance returns multi-index columns if downloading multiple tickers, but we only care about 'Close'
    ...
    mock_df = pd.DataFrame()
    # Setting up the multi-index is tricky, we can just patch so that df["Close"] returns mock_close_df
