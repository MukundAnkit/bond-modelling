...
class CorporateFetcher(DataFetcher):
    ...
    def fetch_yield_curve(self, date: datetime.date) -> dict[float, float]:
        ...
        """  # noqa: D401, E501, W293
        logger.info(
            f"Fetching Corporate {self.rating} index yield from FRED for date {date}"
        )  # noqa: E501
        ...
        try:
            ...
            if pd.isna(latest_yield):
                raise DataFetchError(
                    f"Corporate yield data is NaN for {self.series_id} around {date}"
                )  # noqa: E501
            ...
            raise DataNormalizationError(
                f"Error normalizing Corporate yield data: {e}"
            ) from e  # noqa: E501
