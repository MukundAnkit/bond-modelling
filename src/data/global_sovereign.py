...
class GlobalSovereignFetcher(DataFetcher):
    ...
    def __init__(self, region: str = 'EUR'):
        ...
        if self.region not in self.SERIES_MAP:
            raise ValueError(f"Region {self.region} not supported. Use one of {list(self.SERIES_MAP.keys())}")  # noqa: E501
    ...
    def fetch_yield_curve(self, date: datetime.date) -> dict[float, float]:
        ...
        """  # noqa: D401, W293
        logger.info(f"Fetching 10-year Sovereign Yield for {self.region} from FRED for date {date}")  # noqa: E501
        ...
        if df.empty:
            raise DataFetchError(f"No sovereign data returned from FRED for {self.series_id} up to {date}")  # noqa: E501
        ...
        try:
            ...
            if pd.isna(latest_yield):
                raise DataFetchError(f"Sovereign yield data is NaN for {self.series_id} around {date}")  # noqa: E501
            ...
            if isinstance(e, DataFetchError):
                raise
            raise DataNormalizationError(f"Error normalizing Global Sovereign data: {e}") from e  # noqa: E501
