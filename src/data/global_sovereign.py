...
class GlobalSovereignFetcher(DataFetcher):
    ...
    def __init__(self, region: str = 'EUR'):
        ...
        if self.region not in self.SERIES_MAP:
            raise ValueError(f"Region {self.region} not supported. Use one of {list(self.SERIES_MAP.keys())}")
    ...
    def fetch_yield_curve(self, date: datetime.date) -> Dict[float, float]:
        ...
        """
        logger.info(f"Fetching 10-year Sovereign Yield for {self.region} from FRED for date {date}")
        
        # Monthly data requires a wider lookback window (e.g. 60 days) to find the most recent month
        ...
        if df.empty:
            raise DataFetchError(f"No sovereign data returned from FRED for {self.series_id} up to {date}")
        ...
        try:
            ...
            if pd.isna(latest_yield):
                raise DataFetchError(f"Sovereign yield data is NaN for {self.series_id} around {date}")
            ...
            if isinstance(e, DataFetchError):
                raise
            raise DataNormalizationError(f"Error normalizing Global Sovereign data: {e}") from e
