...
class TreasuryFetcher(DataFetcher):
    """Fetches daily Treasury par yield curve rates from the official US Treasury Fiscal Data API."""
    ...
    def fetch_yield_curve(self, date: datetime.date) -> Dict[float, float]:
        ...
        """
        # The API allows querying exact dates. If a date is a holiday, it returns an empty array.
        ...
        try:
            ...
            raise DataNormalizationError(f"Failed to parse Treasury JSON response: {e}") from e
        ...
        for field, maturity in self.FIELD_MAPPING.items():
            ...
            try:
                ...
                raise DataNormalizationError(f"Could not convert Treasury value '{val}' for {field} to float") from e

        if not curve:
            raise DataNormalizationError("No valid yields found in Treasury API response.")
